...
protected:
    int days;
public:
    date(string _name, string _cname, int _days) : days(_days), country(_name, _cname), prods(_name) { cout << ">_construct111;\n"; }
    ~date() {};

    void date_out() {
        cout << "Expiry date: " << days << " days" << endl;
    }
};
...
public:
    price(string _name, string _cname, string _consistency, string _conds, string _mon_unit, int _days, int _mon) :
        mon(_mon), mon_unit(_mon_unit), strg(_name, _consistency, _conds), date(_name, _cname, _days), prods(_name) {
        cout << ">_construct_UwU;\n";
    }
    ~price() {};
